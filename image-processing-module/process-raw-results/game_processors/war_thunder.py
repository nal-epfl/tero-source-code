...
class WarThunder(Processor):
    ...
    def check_if_ping(self, text):
        return ("pi" in text.lower() or "ng" in text.lower())


    def decide_candidates(self, original, candidates):       
        for candidate in candidates:
            if self.check_if_ping(original[candidate["location"]]) or (candidate["location"] > 0 and self.check_if_ping(original[candidate["location"] - 1])):
