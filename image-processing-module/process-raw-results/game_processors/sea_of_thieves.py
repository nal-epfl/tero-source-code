...
class SeaOfThieves(Processor):
    ...
    def analyze_single_value(self, value):
        contains_ping = [x for x in value if "pi" in x["original"].lower() or "ng" in x["original"].lower() or "in" in x["original"].lower()]
        if contains_ping:
    ...
    def get_values(self, values_to_parse):
        ...
        for group in grouped:  
            ...
            if latency:
                ...
                to_return["has_mark"] = "pi" in whole_text or "ng" in whole_text or "in" in whole_text
