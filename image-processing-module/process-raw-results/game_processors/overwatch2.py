...
class Overwatch2(Processor):
    ...
    def check_if_ms(self, text):
        return "ms" in text.lower() and "i" not in text.lower() and "d" not in text.lower() and "f" not in text.lower() and "p" not in text.lower()


    def decide_candidates(self, original, candidates):       
        for candidate in candidates:
            if  "ind" in original[candidate["location"]].lower() or (candidate["location"] > 0 and ("ind" in original[candidate["location"] - 1].lower())):
                continue

            if self.check_if_ms(original[candidate["location"]]) or (candidate["location"] + 1 < len(original) and self.check_if_ms(original[candidate["location"] + 1]) and "lat" not in original[candidate["location"] + 1].lower()):
